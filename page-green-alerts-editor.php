...
/**
 * This section will handle all the data import for each uploaded Green Alert image
 */ 
...
$post_data = str_replace("%3A", ":", $post_data);
...
*/
	...
	$tmpPath = "wp-content/themes/charlotte_dancesport/ga_uploads/";
	$path = "wp-content/themes/charlotte_dancesport/green_alert_images/";
	$msg = '{';
/*	
	$query = "SELECT green_alert_banner_id FROM cds_green_alert_banners WHERE archived = 0"
	$select = mysql_query($query) or die("Select from cds_member failed.");
	$current_green_alert_id = mysql_fetch_array($select, MYSQL_ASSOC);
*/	
		
		$query = "UPDATE cds_green_alert_banners SET archived=1 WHERE archived=0";
		$update = mysql_query($query) or die("UPDATE into cds_green_alert_banners failed :( boo hoo");
		
		for($i=0; $i < count($filename); $i++) {
			$query = "INSERT INTO cds_green_alert_banners (path,filename,url,title,archived,start_date,end_date,forever,adjudicator_id) 
					  VALUES ('{$path}', '{$filename[$i]}', '{$url[$i]}', '";
			$query .= (isset($title[$i])) ? $title[$i] : "Unknown title";
			$query .= "' , ";
			$query .= ($current==$i) ? "0, " : "1, ";
			...
			$query .= ", {$adjudicator})";
			$insert = mysql_query($query) or die("INSERT into cds_green_alert_banners failed :( boo hoo!");
			
			if( $insert == true ) {
				if( !rename($tmpPath . $filename[$i], $path . $filename[$i]) ) {
					$msg .= '"msg":"Oopsy >.< the image '.$filename[$i].' could not be moved to the green_alert_images folder.","err":"true"';
				} else {
					$msg .= '"msg":"The photo move to green_alert_images/ was successful","err":"false"';
				}
			} else { $msg .= '"msg":"insert failed","err":"true"'; }
	
		}
		/*
		$query = "UPDATE cds_green_alert_banners SET archived=1 WHERE green_alert_banner_id={$image_id}";
		$update = mysql_query($query) or die("UPDATE into cds_green_alert_banners failed :( boo hoo");
		*/
	$msg .= '}';
	echo $msg;
 ...
 *  {string} title is the name over the owner of the image
 ...
 */
function ajax_update_photo_details(imageID, title, start_date, end_date, forever)
{
var httpxml;
try
{
...
url=url+"?imageID="+imageID+"&title="+title+"&start_date="+start_date+"&end_date="+end_date+"&forever="+forever+"&green_alert=true";
...
url=url+"?imageID="+imageID+"&green_alert=true";
url=url+"&sid="+Math.random();
httpxml.onreadystatechange=stateck;
httpxml.open("GET",url,true);
httpxml.send(null);
...
function validURL(url) {
	var filter = /^(((ht|f){1}(tp|tps){1}(:[\/][\/]){1})|((www.){1}))[-a-zA-Z0-9@:%_\+.~#?&\/\/=]+$/;
	if (!filter.test(url)) {
		return false;
	}
	return true;
...
$(document).ready(function() {
	$("#photo_details").submit( function() {
		...
		$(this).find('input').each( function() {
			...
			if( ($(this).attr('id')).match("url") !== null && 
				$(this).val() !== "" && $(this).val() !== null && !validURL($(this).val()) ) {
		...
		if( empty_fields > 0 ) {
			...
			$("current_T").val($('input:radio[name=current]:checked').val());
...
<body>
	<div id="content" style="width:600px;" role="main">
		...
		<div id="header" style="width:660px;">
			...
			<div id="nav_tabs_container" style="width:260px;">
				<ul>
...
switch ($_GET['edit']) {
	case 'upload':
		$filename = get_directory_listing('wp-content/themes/charlotte_dancesport/ga_uploads');
?>

		<div class="cookie_crumb" style="width:600px;"><p><a href="">Edit</a> &gt; Upload photos</p></div>

<?php
		if ( count($filename)>0 && $_GET['step']!=2) {		
?>

		<div id="yellow_alerts">You have <?php echo count($filename); ?> incomplete green alert banner upload<?php if (count($filename) > 1) echo "s";?>! - <a href="?edit=upload&step=2" style="font-size:.7em;">Complete now</a></div>
...
<?php   } ?>
		...
		<div id="upload_container">
			<div class="header_box"><p>Upload Image</p></div>
			...
			?>
				...
				<p class="bold">STEP 1 - Select image to upload</p>
				<ul>
				  <li>Image must be 960x168 pixels.</li>
                ...
                $(document).ready(function() {
                    $("#uploadify").uploadify({
                        ...
                        'script'         : '<?php bloginfo('stylesheet_directory'); ?>/js/ga_uploadify.php',
                        'cancelImg'      : '<?php bloginfo('stylesheet_directory'); ?>/img/cancel.png',
                        'folder'         : '../../wp-content/themes/charlotte_dancesport/ga_uploads',
                        'queueID'        : 'fileQueue',
                        'queueSizeLimit' : 10,
                        'simUploadLimit' : 1,
                        'auto'           : false,
			...
			?>
            	...
            	<p class="bold">STEP 2 - Add image details</p>
				<ul>
				  <li>Give the image a title of your choosing.</li>
				  <li>Select how long to display the photo.</li>
				  <li style="display:none; visibility:hidden;"><small style="color:#999;">* All fields are required!</small></li>
				</ul>
                <form id="photo_details" > <!-- action="<?php //echo str_replace("?".$_SERVER['QUERY_STRING'], "", CurrentPageURL()); ?>" method="post"> -->
            <?php
					/* Added by Lance Newby 8/31/2010 - Store new image data in MySQL DB */
	        ...
	        <input type="hidden" value="<?php echo $fn; ?>" id="filename-<?php echo $counter; ?>" name="filename-<?php echo $counter; ?>" class="filename" />
                <div id="photo-<?php echo $counter; ?>" class="photo_details">
                    <div><a href="<?php bloginfo('stylesheet_directory'); ?>/ga_uploads/<?php echo $fn; ?>" rel="banner_image" alt="<?php echo $fn; ?>" title="<?php echo $fn; ?>"><?php echo $fn; ?></a></div>
                    <label for="title-<?php echo $counter; ?>">Title</label>
                    <input type="text" id="title-<?php echo $counter; ?>" name="title-<?php echo $counter; ?>" class="title" />
                    <label for="url-<?php echo $counter; ?>">Link</label>
                    <input type="text" id="url-<?php echo $counter; ?>" name="url-<?php echo $counter; ?>" class="url" />
                    ...
                    <input type="radio" name="current" value="<?php echo $counter; ?>" <?php if($counter==0) echo 'checked="checked"'; ?> />
                    <label for="current">make viewable</label>
                    <span id="remove-<?php echo $counter; ?>" class="remove_link">remove</span>
                </div>
                <script type="text/javascript" language="javascript">
                $(document).ready(function() {
					$("#url-<?php echo $counter; ?>").focusout( function () {
						// Add 'http://' to the entered url if needed
						if( $(this).val().indexOf("http://") == -1 && $(this).val().indexOf("https://") == -1 && ($(this).val() !== "" && $(this).val() !== null) ) {
			...
			?>
                    ...
                    <input type="hidden" value="" id="current_T" name="current_T" />
	       	    </form>
            <?php
			} else {
			?>
...
?>
			...
			*/
			...
			$('#current_container .edit').click( function () {
				...
				$("#current_home input:checked").each( function() {
					...
					var title=$(this).parents(".single_img_container").find("input[name=title]").val();
					...
					if( forever == 0 ) {
						...
						$(".details_box").append(''
							+'<div id="photo-'+thisID+'" class="photo_edit photo_details">'
								...
								+'<label for="title-'+thisID+'" id="title_lbl-'+thisID+'">Title</label>'
								+'<input type="text" id="title-'+thisID+'" name="title-'+thisID+'" value="'+title+'" class="title" />'
						...
						$("#start_date-"+thisID).focusout( function() {
							if( $(this).val() > $("#end_date-"+thisID).val() && $("#end_date-"+thisID).val() != "" ) {
								...
								var new_title=$("#title-"+thisID).val();
								...
								ajax_update_photo_details(thisID, new_title, new_start_date, new_end_date, new_forever);
						...
						//$("#title_lbl-"+thisID).addClass("title_lbl");

						$("#remove-"+thisID).click( function() {
							ajax_delete_photo("../green_alert_images",filename);
							ajax_delete_photo("../ga_thumbs",filename);
						...
						$("#remove-"+thisID).click( function() {
							ajax_delete_photo("../green_alerts_images",filename);
							ajax_delete_photo("../ga_thumbs",filename);
							ajax_delete_photo_from_db(thisID);
							$("#photo-"+thisID).addClass("photoRemoval").fadeOut(1000, function() { $(this).remove()});
						});
		...
		<div class="cookie_crumb" style="width:600px;"><p><a href="">Edit</a> &gt; Current banner photos</p></div>
...
<?php
	   ...
	   */
	  $query = "SELECT title as title, green_alert_banner_id as id, filename, forever, start_date, DATE_FORMAT(start_date,'%m/%e/%Y') as startDate, end_date, DATE_FORMAT(end_date,'%m/%e/%Y') as endDate FROM cds_green_alert_banners WHERE archived=0 AND (forever=1 OR (start_date <= NOW() && end_date >= NOW()))";
	  $select = mysql_query($query) or die("SELECT for green alert image marked to display failed");
?>
			<div id="current_home">
...
<?php
	if( mysql_num_rows($select) != 0 ) {
	  while($row = mysql_fetch_array( $select )) {
	    ...
	    echo '/ga_thumbs/'.$row["filename"].'" /><div class="checkbox_container"><input type="checkbox" id="'.$row['id'].'" /></div><div class="image_details_container"><p style="float: left; width:230px;" class="filename"><strong>'.$row["filename"].'</strong></p><p style="float: left; width:230px;">Displaying</p>';
	   ...
	   */
	  $query = "SELECT title as title, green_alert_banner_id as id, filename, forever, start_date, DATE_FORMAT(start_date,'%m/%e/%Y') as startDate, end_date, DATE_FORMAT(end_date,'%m/%e/%Y') as endDate FROM cds_green_alert_banners WHERE archived=0 AND forever=0 AND (start_date > NOW() && end_date > NOW())";
	  $select = mysql_query($query) or die("SELECT for green alert images marked to display in the future failed");
?>
            ...
            <div id="current_editor" class="hidden">
            	<div id="changes_saved_container"><div id="changes_saved_alert" class="hidden">Saved</div></div>
           		<div id="upload_container" style="margin-left:0px;">
                    <div class="header_box"><p>Update image details</p></div>
...
?>
        ...
        $(document).ready(function() {
			$('#archive_container .make_current').click( function () {
				...
				// Archive / hide the currently displaying green alert image
				ajax_archive_photo( $(".current_ga_image").val() );
					...
					if( forever == 0 ) {
						...
						$("#photo_details").append(''
							+'<div id="photo-'+thisID+'" class="photo_edit photo_details">'
								...
								+'<input type="hidden" class="title" name="title-'+thisID+'" id="title-'+thisID+'" value="'+title+'">'
								+'<span id="duration-'+thisID+'" class="duration_span">'
								+'<label for="start_date-'+thisID+'" class="duration_lbl">Duration</label>'
								+'<input type="text" id="start_date-'+thisID+'" name="start_date-'+thisID+'" value="'+convert_date(start_date)+'" size="9" /> '
								+'<label for="end_date">to</label> '
			...
			$("#current_home input[type=checkbox]").click(function() {
			   	...
			   	if( $(this).is(":checked") ) {
					...
					// Make all appropriate edit buttons active.
		...
		<div class="cookie_crumb" style="width:600px;"><p><a href="">Edit</a> &gt; <a href="?edit=archive">Add from archive</a></p></div>

<?php  // Store temporarily the id of the currently displaying green alert images
		$query = "SELECT green_alert_banner_id as id FROM cds_green_alert_banners WHERE archived=0";
		$select = mysql_query($query) or die("SELECT for green alert images marked as unarchived failed");
		$current_image = mysql_fetch_array($select);
?>
        ...
        <div id="archive_container" class="edit_tool_content">
            <div class="edit_toolbar">
                ...
                <input type="button" value="Delete" name="delete" class="delete" disabled="disabled" />
                <input type="hidden" value="<? echo $current_image["id"]; ?>" class="current_ga_image" />
                <span class="edit_buttons hidden">
                    <label for="saved"></label>
                    <input type="button" value="Save" name="save" class="save" id="save" disabled="disabled" />
                    <Input type="button" value="Done" name="done" class="done" />
...
<?php  // Get all archived green alert images
		$query = "SELECT title as title, green_alert_banner_id as id, filename, forever, start_date, DATE_FORMAT(start_date,'%m/%e/%Y') as startDate, end_date, DATE_FORMAT(end_date,'%m/%e/%Y') as endDate FROM cds_green_alert_banners WHERE archived=1";
		$select = mysql_query($query) or die("SELECT for green alert images marked as archived failed");
?>
			<div id="current_home">
<?php
	  if( mysql_num_rows($select) != 0 ) {
	  ...
	  } else { echo '<div class="no_images">Currently there are no images archived :-p</div>'; }
?>
			</div>
