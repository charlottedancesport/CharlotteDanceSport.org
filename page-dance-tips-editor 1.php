...
/**
 * Add new dance tip to database
 *
 * @param {string} dance_tip is the text for the tip 
 *  {integer} user_id is the wordpress user id for the currently logged in user.
 */
function ajax_add_new_tip(dance_tip, user_id)
{
var httpxml;
try
{
...
if(httpxml.readyState==4)
{
	if(httpxml.status == 200){
		$(document).ready( function () {
			...
			/* 
			 * Display a border around dance tip text when mouse hovers over it
			 */
			...
			$("#current_home input[type=checkbox]").click(function() {
				// If a checkbox is check for a dance tip make appropriate edit buttons enabled.
				if( $(this).is(":checked") ) {
...
var url="<?php bloginfo('url'); ?>/edit_tools/ajax-add-new-dance-tip/";
url=url+"?dance_tip="+dance_tip+"&submitter_id="+user_id;
...
/**
 * Delete selected dance tips
 *
 * @param {string} dance_tip is the text for the tip 
 *  {integer} user_id is the wordpress user id for the currently logged in user.
 */
function ajax_delete_dance_tip(tip_id)
{
var httpxml;
try
{
...
var url="<?php bloginfo('url'); ?>/edit_tools/ajax-delete-dance-tip/";
url=url+"?tip_id="+tip_id;
url=url+"&sid="+Math.random();
httpxml.onreadystatechange=stateck;
httpxml.open("GET",url,true);
httpxml.send(null);
...
$(document).ready( function() {
	...
	/*
	 * Select all dance tips checkboxes
	 */
		...
		function(event)
		{
			...
			if (charCode == 13) {
				if( $(this).val() == "" || $(this).val() == null ) {
					...
					ajax_add_new_tip( $(this).val(), <? echo (is_user_logged_in()) ? $current_user->ID : "0"; ?> );
					$(this).val("");
					$(this).blur();
...
<body>
	<div id="content" style="width:600px;" role="main">
		...
		<div class="cookie_crumb" style="width:600px;"><p><a href="">Edit</a> &gt; Dance Tips</p></div>
